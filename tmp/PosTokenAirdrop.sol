...
contract POSDuckToken is ERC20, Ownable {
    ...
    function addMinter(address minterAddress) external onlyOwner {
        require(
            Address.isContract(minterAddress),
            "Only contract may be a minter"
        );
...
contract POSDuckAirdrop is Ownable, Pausable {
    ...
    constructor(address oracle_, address tokenContractAddress_) {
        ...
        require(
            Address.isContract(tokenContractAddress_),
        ...
        whenNotPaused
    {
        ...
        bytes32 hash = keccak256(
            abi.encodePacked(msg.sender, "pos_duck_airdrop", amount)
        );
        require(
        ...
        POSDuckToken(tokenContractAddress).mint(msg.sender, amount);
