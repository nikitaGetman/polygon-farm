...
interface CommonInterface {
    ...
    function getBoost(uint256 boostId)
        external
        view
        returns (Boosts.Boost memory boost);
...
contract POSDuckSwap is Ownable, Pausable, IERC721Receiver {
    ...
    constructor(
        address oracle_,
        address posduckContractAddress_,
        address galaxyContractAddress_
    ) {
        require(oracle_ != address(0x0), "Invalid oracle address");
        require(
            Address.isContract(posduckContractAddress_),
            "Invalid contract address"
        );
        require(
            Address.isContract(galaxyContractAddress_),
            "Invalid contract address"
        );
    ...
    function claim(uint256 galaxyTokenId, bytes calldata signature)
        external
        whenNotPaused
    {
        bytes32 hash = keccak256(
            abi.encodePacked(msg.sender, "-", galaxyTokenId)
        );
        require(
            signerAddress(prefixed(hash), signature) == oracle,
            "Invalid signature"
        );

        require(
            receivedBoosts[galaxyTokenId] == address(0x0),
            "Token already swapped"
        );
        receivedBoosts[galaxyTokenId] = msg.sender;

        CommonInterface(GALAXY_CONTRACT_ADDRESS).safeTransferFrom(
            msg.sender,
            address(this),
            galaxyTokenId
        );
        ...
        CommonInterface(POSDUCK_CONTRACT_ADDRESS).safeTransferFrom(
            address(this),
            msg.sender,
            id
        );
    ...
    function splitSign(bytes memory sig)
        internal
        pure
        returns (
            uint8 v,
    ...
    function signerAddress(bytes32 message, bytes memory sig)
        internal
        pure
        returns (address)
    {
    ...
    function getID() private view returns (uint256) {
        ...
        while (
            CommonInterface(POSDUCK_CONTRACT_ADDRESS).ownerOf(id) !=
            address(this)
        ) {
            id++;
        }
